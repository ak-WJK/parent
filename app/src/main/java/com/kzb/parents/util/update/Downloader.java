...
public class Downloader {
	 ...
	 * 保存记录文件
	 */
	protected synchronized boolean saveLogFile() {
	 ...
	 * @param needReDownload 如果文件已经存在是否需要重新下载
	 */
	public Downloader(Context context, String downloadUrl, File dir, int threadNum,boolean needReDownload,String versionCode) {
		try {
...
//			print("dir="+dir);
			...
			//fileService = FileService.getInstance(context);
...
//			String filename = getFileName(conn);
			String filename;
			filename = versionCode+"_"+this.downloadUrl.substring(this.downloadUrl.lastIndexOf('/') + 1);
//			print("filename="+filename);
			this.saveFile = new File(dir, filename);/* 保存文件 */
			
			//如果needReDownload为false且文件已经下载,则不继续操作  否则删除文件 继续操作
			if(!needReDownload){
				if(getExistFileLength()==this.fileSize){
					this.finish=true;
					return;
				}
			}else{
	...
	public void stopDownload(){
		for (int i = 0; i < this.threads.length; i++){
			if(this.threads[i]!=null){
				this.threads[i].isNeedStop=true;
			}
		}
	}
