    ...
    public void run() {
        if (downLength < block) {//未下载完成
            try {
                ...
                while ((offset = is.read(buffer, 0, BUFFER_SIZE)) != -1 && !isNeedStop && isdbsuccess) {
                    raf.write(buffer, 0, offset);
                    downLength += offset;
                    downloader.update(this.threadId, downLength);
                    isdbsuccess = downloader.saveLogFile();
                ...
                if (!isdbsuccess) {//操作数据库失败
                    print("Thread " + this.threadId + " has stop,db error");
                    this.finish = false;
                } else {
                    if (!isNeedStop) {
                        ...
                        print("Thread " + this.threadId + " has stop");
                        this.finish = false;
                    }
                }
