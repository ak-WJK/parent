...
//        mRates.add(rate);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        RectF rectF = new RectF(tX-DensityUtil.dip2px(getContext(),140),tY-DensityUtil.dip2px(getContext(),10),tX-DensityUtil.dip2px(getContext(),100),tY+DensityUtil.dip2px(getContext(),10));
...
//        canvas.drawLine(tX- DensityUtil.dip2px(getContext(), 140), tY, tX- DensityUtil.dip2px(getContext(), 100), tY, paint);
        ...
        canvas.drawText("正确数",tX-DensityUtil.dip2px(getContext(),95),tY+DensityUtil.dip2px(getContext(),6),paint);
...
//        canvas.drawLine(tX, tY, tX + DensityUtil.dip2px(getContext(), 40), tY, paint);
        paint.setColor(texta);

        paint.setTextSize(DensityUtil.dip2px(getContext(),15));
        canvas.drawText("错误数",tX+DensityUtil.dip2px(getContext(),45),tY+DensityUtil.dip2px(getContext(),6),paint);
    }
...
//            canvas.drawCircle(x, y, DensityUtil.dip2px(mContext,3), paint);
            //放label线的长度
            float labelL = DensityUtil.dip2px(mContext,90);
            if((startAngle+angle/2)<90){
                ...
                //写字
                float tx = zX + labelL -DensityUtil.dip2px(mContext,80);
                float ty = zY;
...
//        canvas.drawArc(oval2, 230, 130, true, paint);
    ...
    public void drawInnerRing(Canvas canvas) {
        ...
        canvas.drawCircle(mWidth / 2, mHeight / 2+mTopMargin, mRadius - DensityUtil.dip2px(mContext, 35), paint);
    }
                    ...
                    if (whe > rate) {
                        switch (i){
                            case 0:
                                ...
                                mapVal.put("name","错误题目");
                                mapVal.put("typev","1");
//                                IntentUtil.startActivity((BaseActivity)mContext,ReportFinishActivity.class,mapVal);
                                break;
                            case 1:
                                mapVal = new HashMap<String, String>();
                                mapVal.put("test_id",test_id);
                                mapVal.put("name","正确题目");
                                mapVal.put("typev","2");
//                                IntentUtil.startActivity((BaseActivity)mContext,ReportFinishActivity.class,mapVal);
                                break;
