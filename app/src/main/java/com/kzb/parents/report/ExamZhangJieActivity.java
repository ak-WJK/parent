...
import com.kzb.parents.report.adapter.ExamZhangJieAdapter;
...
public class ExamZhangJieActivity extends BaseActivity implements View.OnClickListener{

    private TextView titleLeft, titleCenter;
    private ExpandableListView expandableListView;
    ExamZhangJieAdapter mZjAdapter;

    @Override
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_exam_zhangjie);

        httpConfig = new HttpConfig();
        dialogView = new DialogView(ExamZhangJieActivity.this);
        initView();
        initData();
    }
    ...
    protected void initData() {
        getZjRecord(AddressConfig.ZJ_REPORT_LIST);
    }
...
//                dialogView.handleDialog(false);
                if (response.errorCode == 0) {
                    if (response.getContent() != null) {
                            mZjAdapter = new ExamZhangJieAdapter(ExamZhangJieActivity.this,response.getContent());
//                            mZjAdapter.setType("exam");
                            expandableListView.setAdapter(mZjAdapter);
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            case R.id.common_head_left:
                IntentUtil.finish(ExamZhangJieActivity.this);
                break;
        }
    }
}
